...
set_position!(body::AbstractBody{T}, position::GB.Vec{2, T}) where {T} = body.position = position
get_position_change(body::AbstractBody) = body.position_change
set_position_change!(body::AbstractBody{T}, position_change::GB.Vec{2, T}) where {T} = body.position_change = position_change
add_position_change!(body::AbstractBody{T}, position_change::GB.Vec{2, T}) where {T} = set_position_change!(body, get_position_change(body) .+ position_change)
...
set_velocity!(body::AbstractBody{T}, velocity::GB.Vec{2, T}) where {T} = body.velocity = velocity
get_velocity_change(body::AbstractBody) = body.velocity_change
set_velocity_change!(body::AbstractBody{T}, velocity_change::GB.Vec{2, T}) where {T} = body.velocity_change = velocity_change
add_velocity_change!(body::AbstractBody{T}, velocity_change::GB.Vec{2, T}) where {T} = set_velocity_change!(body, get_velocity_change(body) .+ velocity_change)
