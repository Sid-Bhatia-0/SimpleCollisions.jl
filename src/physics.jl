...
function get_normal_impulse(inv_mass_a::T, inv_mass_b::T, initial_velocity_ao::SA.SVector{2, T}, initial_velocity_bo::SA.SVector{2, T}, e::T, normal_o::SA.SVector{2, T}) where {T}
...
get_normal_impulse(inv_mass_b::T, initial_velocity_ao::SA.SVector{2, T}, initial_velocity_bo::SA.SVector{2, T}, e::T, normal_o::SA.SVector{2, T}) where {T} = get_normal_impulse(zero(T), inv_mass_b, initial_velocity_ao, initial_velocity_bo, e, normal_o)

# dynamic body vs. static body
get_normal_impulse(inv_mass_b::T, initial_velocity_bo::SA.SVector{2, T}, e::T, normal_o::SA.SVector{2, T}) where {T} = get_normal_impulse(zero(T), inv_mass_b, zero(SA.SVector{2, T}), initial_velocity_bo, e, normal_o)

function get_tangential_impulse(inv_mass_a::T, inv_mass_b::T, initial_velocity_ao::SA.SVector{2, T}, initial_velocity_ba::SA.SVector{2, T}, tangent_o::SA.SVector{2, T}, mu_s::T, mu_k::T, j_ao_normal_o::T) where {T}
    initial_velocity_ba = initial_velocity_bo .- initial_velocity_ao
