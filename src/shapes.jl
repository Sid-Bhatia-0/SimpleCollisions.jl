...
get_head(line::StandardLine, pos::Vector2D) = pos + get_head(line)
get_tail(line::StandardLine, pos::Vector2D) = pos + get_tail(line)
get_vertices(line::StandardLine, pos::Vector2D) = (get_tail(line, pos), get_head(line, pos))

# head, tail, and vertices for StandardLine at arbitrary position and orientation
get_head(line::StandardLine, pos::Vector2D, dir::Vector2D) = pos + get_half_length(line) * dir
get_tail(line::StandardLine, pos::Vector2D, dir::Vector2D) = pos - get_half_length(line) * dir
get_vertices(line::StandardLine, pos::Vector2D, dir::Vector2D) = (get_tail(line, pos, dir), get_head(line, pos, dir))
...
get_bottom_left(rect::StandardRect, pos::Vector2D) = pos + Vector2D(-get_half_width(rect), -get_half_height(rect))
get_bottom_right(rect::StandardRect, pos::Vector2D) = pos + Vector2D(get_half_width(rect), -get_half_height(rect))
get_top_right(rect::StandardRect, pos::Vector2D) = pos + Vector2D(get_half_width(rect), get_half_height(rect))
get_top_left(rect::StandardRect, pos::Vector2D) = pos + Vector2D(-get_half_width(rect), get_half_height(rect))
get_vertices(rect::StandardRect, pos::Vector2D) = (get_bottom_left(rect, pos), get_bottom_right(rect, pos), get_top_right(rect, pos), get_top_left(rect, pos))

# bottom_left, bottom_right, top_left, top_right, and vertices for StandardRect at arbitrary position and orientation
get_bottom_left(rect::StandardRect, pos::Vector2D, dir::Vector2D) = pos - get_half_width(rect) * dir - get_half_height(rect) * rotate_plus_90(dir)
get_bottom_right(rect::StandardRect, pos::Vector2D, dir::Vector2D) = pos + get_half_width(rect) * dir - get_half_height(rect) * rotate_plus_90(dir)
get_top_right(rect::StandardRect, pos::Vector2D, dir::Vector2D) = pos + get_half_width(rect) * dir + get_half_height(rect) * rotate_plus_90(dir)
get_top_left(rect::StandardRect, pos::Vector2D, dir::Vector2D) = pos - get_half_width(rect) * dir + get_half_height(rect) * rotate_plus_90(dir)
get_vertices(rect::StandardRect, pos::Vector2D, dir::Vector2D) = (get_bottom_left(rect, pos, dir), get_bottom_right(rect, pos, dir), get_top_right(rect, pos, dir), get_top_left(rect, pos, dir))
...
function get_normals(rect::StandardRect, dir::Vector2D)
    x_cap = dir
