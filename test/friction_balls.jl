...
shape1 = GB.HyperSphere(GB.Point2{T}(5.0f0, 3.0f0), one(T))
material_data1 = PE2D.MaterialData{T}()
mass_data1 = PE2D.MassData(material_data1.density, shape1)
position_accumulator1 = PE2D.Accumulator(GB.Vec2{T}(5.0f0, 3.0f0), zero(GB.Vec2{T}))
velocity_accumulator1 = PE2D.Accumulator(GB.Vec2{T}(0.0f0, 0.0f0), zero(GB.Vec2{T}))
...
angular_velocity_accumulator1 = PE2D.Accumulator(zero(T), zero(T))
...
shape2 = GB.HyperSphere(GB.Point2{T}(2.0f0, 8.0f0), one(T))
material_data2 = PE2D.MaterialData{T}()
mass_data2 = PE2D.MassData(material_data2.density, shape2)
position_accumulator2 = PE2D.Accumulator(GB.Vec2{T}(2.0f0, 8.0f0), zero(GB.Vec2{T}))
velocity_accumulator2 = PE2D.Accumulator(GB.Vec2{T}(1.0f0, -1.0f0), zero(GB.Vec2{T}))
...
angular_velocity_accumulator2 = PE2D.Accumulator(zero(T), zero(T))
